...
TravelPlanner je kompleksna ASP.NET Core aplikacija sa **mikroservisnom arhitekturom** koju pokre?e **Azure Service Fabric**. Aplikacija omogu?ava korisnicima da planiraju putovanja, prate bud�ete, upravljaju destinacijama, aktivnostima i delim planove sa drugim korisnicima.
...
- **BCrypt** - He�iravanje lozinki
- **CORS** - Podr�ka za React frontend
...
```
POST /api/planputovanja/trosak - Dodavanje tro�ka u plan
PUT /api/planputovanja/trosak/{id} - Izmena tro�ka
DELETE /api/planputovanja/trosak/{id} - Brisanje tro�ka
GET /api/planputovanja/{id}/potrosnja - Preuzimanje trenutne potro�nje
```

**Upravljanje To-Do Stavkama:**
```
POST /api/planputovanja/todo - Dodavanje stavke na listu
PUT /api/planputovanja/todo/{id}/toggle - Ozna?avanje stavke kao zavr�ene/nezavr�ene
...
- `GetPlanPutovanjaSaDetaljimaAsync()` - Preuzimanje plana sa svim relacijama (destinacije, tro�cima, To-Do stavkama)
- `AzurirajPlanPutovanjaAsync()` - A�uriranje (Naziv, Opis, PlaniraniBudzet)
- `ObrisiPlanPutovanjaAsync()` - Brisanje plana

**Destinacije:**
- `AddDestinacijaAsync()` - Dodavanje destinacije
- `AzurirajDestinacijuAsync()` - A�uriranje (NazivMesta, Napomena, DatumDolaska, DatumOdlaska)
- `ObrisiDestinacijuAsync()` - Brisanje destinacije

**Tro�ci:**
- `AddTrosakAsync()` - Dodavanje tro�ka
- `AzurirajTrosakAsync()` - A�uriranje (Kategorija, Opis, Iznos, Datum)
- `ObrisiTrosakAsync()` - Brisanje tro�ka
- `DobaviSumuTroskovaZaPlanAsync()` - Brzo preuzimanje sume tro�kova (koristi se za ke� u BudgetService)

**To-Do Stavke:**
- `DodajToDoStavkuAsync()` - Dodavanje stavke
- `PromeniStatusStavkeAsync()` - Toggle zavr�enosti
- `ObrisiToDoStavkuAsync()` - Brisanje stavke

**Aktivnosti:**
- `DodajAktivnostAsync()` - Dodavanje aktivnosti
- `AzurirajAktivnostAsync()` - A�uriranje svih polja
- `ObrisiAktivnostAsync()` - Brisanje aktivnosti

**Upravljanje Korisnicima:**
- `RegistrujKorisnikaAsync()` - Registracija (lozinka se he�iruje sa BCrypt)
- `ProverKredencijalAsync()` - Verifikacija lozinke tokom logovanja
- `DobaviSveKorisnikeAsync()` - Preuzimanje svih korisnika (za admin)
- `ObrisiKorisnikaAsync()` - Brisanje korisnika (ne mo�e se obrisati glavni admin ID=1)
...
Brzi in-memory servis koji prati potro�nju bud�eta sa ke� optimizacijom.

#### Tehni?ki Detalji:
- **Tip**: Stateful Service (?uva stanje u memoriji klastera)
- **Stanje**: `IReliableDictionary<int, double>` - Mapira Plan ID ? Trenutna Potro�nja
...
- Inkrementira potro�nju za plan za dati iznos
- Ako plan jo� ne postoji u ke� memoriji, inicijalizuje se sa 0
- Vra?a novu sumu potro�nje
- **Optimizacija**: Particionisano po `PlanId` - svaki plan ide na specifi?nu particiju
  
```
Tok: WebAPI �alje tro�ak ? TravelDataService ?uva u SQL ? BudgetService a�urira ke�
     Za particionisanje: new ServicePartitionKey(trosak.PlanPutovanjaId)
```

**DobaviUkupnuPotrosnjuAsync(int planId)**
- **2-fazna strategija**:
  1. Prvo proverava ke� - ako postoji, odmah vra?a (ultra brzo)
  2. Ako ke� nedostaje (CACHE MISS):
     - Poziva TravelDataService da izra?una sumu iz baze
     - Popunjava ke� sa tim vrednostima
     - Vra?a rezultat

```
Optimizacija: Minimizira SQL upite - ve?ina zahteva radi sa ke� memorijom
```

**InvalidirajKesBudzetaAsync(int planId)**
- Bri�e vrednost iz ke� memorije
- Koristi se kada se tro�ci izmene/bri�u
- Slede?i `DobaviUkupnuPotrosnjuAsync()` ?e ponovljivo izra?unati iz baze

#### Integracija sa drugim servisima:
- **BudgetService ? SharingService**: Kada se doda tro�ak, �alje notifikaciju
...
- Poziva je `BudgetService` kada se dod� tro�ak
- Notifikacija sadr�i: ID plana, iznos, novi bud�et

#### Tok Notifikacija:
```
1. Korisnik doda tro�ak preko WebAPI
2. TravelDataService ?uva u SQL
3. BudgetService a�urira ke� i poziva SharingService
4. SharingService dodaje u red: "Plan 5: Dodat tro�ak od 50. Novi bud�et iznosi: 250"
5. Frontend mo�e polling-ovati SharingService za nove notifikacije
```
...
IBudgetService - Bud�et operacije
...
**Tok logovanja:**
1. Korisnik �alje email + lozinka na `/api/auth/login`
2. TravelDataService verifikuje sa `BCrypt.Verify()`
3. WebAPI generi�e JWT token sa 2-satnim rokom trajanja
4. Frontend ?uva token u localStorage i �alje u svakom zahtevom u `Authorization: Bearer {token}`
...
**Nivoi pristupa:**
```
- KORISNIK  - Mo�e upravljati svojim planovima
- ADMIN     - Ima pristup svim korisnicima i mo�e menjati uloge
```
...
**Karakteristike:**
- Kra?e trajanje (defini�e korisnik pri generisanju)
- Sadr�i samo `PlanId` i `NivoPristupa`
- Druga?iji tajni klju? i issuer od glavnog JWT-a
- `[AllowAnonymous]` endpoint - mo�e pristupiti bilo ko sa validnim tokenom
...
```json
{
  ...
  "TipTokena": "DeljenjePlana",     // Za�tita od reuse-a
  "exp": 1234567890                 // Kratko trajanje
}
```

### He�iravanje Lozinki - BCrypt
...
**Za�to je ovo va�no?**
- Sve operacije za jedan plan idu na istu ma�inu/repliku
- Br�e je jer nema mre�ne latencije izme?u particija
- Ke� ostaje lokalan na particiji

### Tok Dodavanja Tro�ka (End-to-End)
...
Ovo generi�e `Service Fabric Application` koja pokre?e sve:
...
### Scenario 2: Dodavanje Tro�ka i Pra?enje Bud�eta
...
3. WebAPI ? BudgetService (ServicePartitionKey(1))
   ? Inkrementira ke�: bud�et[1] = 500 + 75.50 = 575.50
   ? BudgetService ? SharingService
   ? Notifikacija: "Plan 1: Dodat tro�ak od 75.50. Novi bud�et: 575.50"
...
```
1. Korisnik (ID=5) �eli da deli plan 10 sa prijateljima
...
2. Frontend ? POST /api/planputovanja/10/generisi-token
   ...
   ? WebAPI generi�e JWT sa 1-satnim rokom
   ? Vra?a token: "eyJhbGc..."

3. Korisnik �alje token prijatelju kroz URL:
...
3. Admin ? DELETE /api/admin/korisnici/7
   ? Bri�e korisnika (ali ne mo�e obrisati ID=1 - glavni admin)
```
...
- **BCrypt.Net-Core** - He�irovanje lozinki
- **System.IdentityModel.Tokens.Jwt** - JWT tokenizacija

### Hardkodirani Tajni Klju?evi (BEZBEDNOST!)
?? **VA�NO**: Ovi klju?evi su hardkodirani u razvoju, ALI se MORAJU prebaciti u Secrets ili Azure Key Vault za produkciju:
...
? **Stateful servisi** - BudgetService i SharingService (sa ke� memorijom)
? **Particionisanje** - Br�e ?itanja i pisanja za iste planove
? **RPC komunikacija** - Service Fabric Remoting umesto HTTP-a
? **Sigurnost** - JWT, RBAC, BCrypt he�irovanje
